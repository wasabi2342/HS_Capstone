...
public class PinkPlayerController : ParentPlayerController
{
    ...
    public void CreateUltimateEffectHit()
    {
        ...
        if (animator.GetBool("Right"))
        {
            ...
            headEffectPath = "SkillEffect/PinkPlayer/Ult_HeadParitcle_Right";
        }
        else
        {
            ...
            headEffectPath = "SkillEffect/PinkPlayer/Ult_HeadParitcle_Left";
        }

        if (PhotonNetwork.IsConnected && photonView.IsMine)
        {
            photonView.RPC("CreateAnimation", RpcTarget.Others, effectPath, targetPos, false, animator.speed);
            photonView.RPC("CreateAnimation", RpcTarget.Others, headEffectPath, targetPos, false, animator.speed);
        }
        ...
        // 생성된 이펙트의 부모를 설정 Ult_HeadParitcle_Right
        // skillEffect.transform.parent = transform;
    }
